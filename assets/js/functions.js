...
function redireccionArticulo_Imagenes(id) {
  //funcion de redireccion de articulos
  window.location.href =
    "./../maderastablas/articulo_imagenes.php?img_articleID=" + id;
}
...
$(function () {
  ...
  $("[id^='modificar__Articulo']").click(function (e) {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        $("select option[value=" + data.art_categoria + "]").attr(
          "selected",
          true
        );
  ...
  $("#txt_Cantidad").change(function (e) {
    ...
    if (
      parseInt($("#txt_Cantidad").val()) >= 1 &&
      parseInt($("#txt_Cantidad").val()) <= parseInt($("#txt_Stock").val())
    ) {
  ...
  $("#btn_Calcular-balance").click(function (e) {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (resultadoFinal == 0 || resultadoFinal == 0.0) {
  ...
  //Empieza --- Boton que ELIMINA ARTICULO EN MODAL

  $("#modal-insertarStock-btn").click(function (e) {
    e.preventDefault();
    if (
      $("#modal-insertarStock_cantidad").val().trim() !== "" &&
      $("#modal-insertarStock_concepto").val().trim() !== "" &&
      $("#modal-insertarStock_proveedor").val().trim() !== "" &&
      $("#modal-insertarStock_gasto").val().trim() !== ""
    ) {
